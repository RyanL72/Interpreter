...
(define interpret
  (lambda (filename)
    (let* ((program (parser filename))
           (global-env (interpret-global-level program (newenvironment)))
           (main-func (hash-ref global-env 'main)))
...
; Helper to show what parser returns
(define spitout
  (lambda (filename)
    (parser filename)))

; Helper function to show what interpret top level returns
(define showme
  (lambda (filename)
    (interpret-global-level (parser filename) (newenvironment))))
...
(define interpret-global-level
  (lambda (global-level-list environment)
    (cond
      ((null? global-level-list) environment)
      ((eq? 'class (caar global-level-list)) (insert (cadar global-level-list) (interpret-top-level (cadddr (car global-level-list)) (newenvironment)) environment))
      (else (interpret-global-level (cdr global-level-list) environment)))))

(define interpret-global-levelx
  (lambda (global-level-list environment)
    (cond
      ((null? global-level-list) environment)  ; Base case: return the environment when done
      
      ((eq? 'class (caar global-level-list))  ; If the current item is a class definition
       (let* ((class-definition (cadar global-level-list))  ; Get the class name
              (class-body (cadddr (car global-level-list)))  ; Get the class body (static functions, etc.)
              (new-env (interpret-top-level class-body environment)))  ; Process the class body
         (insert class-definition new-env environment)))  ; Now insert the class into the environment
      
      (else (interpret-global-level (cdr global-level-list) environment)))))  ; Otherwise, continue processing
;'(#hash(( (class A () ((static-function main () ((return 5))))) . #0#)))
                  

;'(#hash( (A . (closure () () ())) ) )
; Get the functions and vars and put them in the closure 
                ...
                (cond
                  ...
                  ((eq? (car stmt) 'function)
                   (let* ((name (cadr stmt))
                          (params (caddr stmt))
                          (body (cadddr stmt)))
                     (make-function-closure name params body environment))) 
...
; Helper method to create a class closure
(define make-class-closure
  (lambda (parent fields methods)
    (list 'closure parent fields methods)))

; Helper method to create an instance closure
