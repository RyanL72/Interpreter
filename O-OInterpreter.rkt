...
(define interpret-global-level
  (lambda (global-level-list environment)
    (if (null? global-level-list)
        environment
        ((let* ((stmt (car global-level-list))
               (new-env
                (cond
                  ((eq? (car stmt) 'class)
                   (let* ((name (cadr stmt))
                          ...
                          (fields '())
                          (methods '())
                          (closure (make-class-closure name parent fields methods)))
                     (insert name closure environment)))
                  (else (myerror "Invalid top-level statement:" stmt)))))
          (interpret-global-level (cdr global-level-list) new-env))))))
    ...
    (if (null? top-level-list)
        ...
        (let* ((stmt (car top-level-list))
               (new-env
                (cond
                  ((eq? (car stmt) 'var)
                   (lookup-in-env (cadr stmt) environment))
                  ((eq? (car stmt) 'function)
                   (let* ((name (cadr stmt))
                          (params (caddr stmt))
                          (body (cadddr stmt))
